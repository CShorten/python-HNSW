...
class experimentalHNSW:
    def __init__(self, max_connections=16, max_layers=2, ef=16, efConstruction=16):
        ...
        '''
        self.node_id_to_filters = {}
        '''
        dicitonary mapping node ids to their respective filters, e.g.
        {
            0: [1], # testing just one filter for now
            1: [1],
            2: [4],
            ...
        }
    ...
    def _insert_node(self, q_id, q, M, M_max, efConstruction, q_filters):
        ...
        q_filters:       []int     -- query filter (just testing 1 filter for now)
        '''
        self.node_id_to_vector[q_id] = q # always going to be adding this node to the graph
        self.node_id_to_filters[q_id] = [q_filters]
        ...
        for lc in range(min(L, l), -1, -1):
            nearest_neighbors = self._search_layer(q_id, ep, efConstruction, lc)
            neighbors_to_add = self._select_neighbors_filter_heuristic(q_id, nearest_neighbors, M, lc)
            ...
            for e in neighbors_to_add:
                ...
                if len(e_conn) > M_max:
                    e_new_conn = self._select_neighbors_filter_heuristic(e, e_conn, M_max, lc)
    ...
    def _select_neighbors_filter_heuristic(self, q_id, C, M, lc, extendCandidates=False, keepPrunedConnections=False):
        '''
        q_id:                      int,            query vector id
        C:                      list of np.arrays, candidate elements
        M:                      int, number of neighbors to return
